...
namespace Navrang.Billing.Infrastructure.Persistence
{
	public class AppDbContext : DbContext
     {
          ...
          [Obsolete]
          public List<ItemEntityModel> GetAllItems()
          {
               try
               {
                    //SqlParameter usernameParam = new SqlParameter("@p1", productId.ToString() ?? (object)DBNull.Value);

                    string sqlQuery = "EXEC [dbo].[getAllItems] ";

                    return this.Query<ItemEntityModel>().FromSql(sqlQuery).ToList();
               }
               catch (Exception ex)
               {
                    return null;
